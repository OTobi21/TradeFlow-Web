...
export default function PortfolioChart() {
  ...
  if (loading) {
    return <SkeletonCard height="h-[320px]" className="p-6" />;
  }
    ...
    <div className="w-full">
      ...
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
            ...
            <XAxis dataKey="date" stroke="transparent" tick={false} axisLine={false} />
      ...
      <div className="mt-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div
            className={`w-3 h-3 rounded-full ${isPositive ? 'bg-green-400' : 'bg-red-400'}`}
          ></div>
        ...
        <div className="text-sm text-tradeflow-muted">
          {data.length > 0 && (
            <>
              Starting: ${data[0]?.value.toLocaleString()} → Current: $
